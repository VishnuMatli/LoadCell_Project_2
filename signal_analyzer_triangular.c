// signal_analyzer_triangular.c: DSP analysis with TRIANGULAR WINDOW applied to FIR coefficients
// Window Type: TRIANGULAR (Bartlett) - Good for moderate spectral leakage reduction
...
// Generate FIR coefficients with TRIANGULAR window
// Triangular window: w(n) = 1.0 - 2.0 * |n - (N-1)/2| / (N-1)
void generate_fir_coeffs_triangular(float* coeffs, int num_taps, 
    ...
    printf("[DSP_TRIANGULAR] Generating FIR coefficients with TRIANGULAR window\n");
    printf("[DSP_TRIANGULAR] Cutoff: %.2f Hz, Sampling: %.2f Hz, Normalized fc: %.4f\n", 
    ...
    for (int n = 0; n < num_taps; n++) {
        ...
        // TRIANGULAR WINDOW: w(n) = 1.0 - 2.0 * |n - (N-1)/2| / (N-1)
        float triangular_window = 1.0f - 2.0f * fabsf((float)n - alpha) / (num_taps - 1);
        
        // Apply window to sinc
        coeffs[n] = sinc_val * triangular_window;
        gain += coeffs[n];
    }
    ...
    printf("[DSP_TRIANGULAR] FIR coefficients generated and normalized\n");
}

// DSP Functions
int normalize_to_weights(const int* values, int num_values, float* weights_out) {
    if (!values || !weights_out || num_values <= 0) {
        fprintf(stderr, "[DSP_TRIANGULAR] Error: Invalid input to normalize_to_weights.\n");
        return -1;
    }
...
int fir_filter(const float* values, int num_values, float* filtered_values_out,
    ...
    if (!values || !filtered_values_out || !fir_coeffs || num_values <= 0) {
        fprintf(stderr, "[DSP_TRIANGULAR] Error: Invalid input to fir_filter.\n");
        return -1;
    }

    if (num_values < NUMTAPS) {
        fprintf(stderr, "[DSP_TRIANGULAR] Warning: Data length (%d) < NUMTAPS (%d). Skipping filter.\n", 
                num_values, NUMTAPS);
        return 0;
    }
    ...
    if (!state) {
        fprintf(stderr, "[DSP_TRIANGULAR] Error: Memory allocation failed for FIR state buffer.\n");
        return -1;
    }
    ...
    // Clean up
    free(state);
    printf("[DSP_TRIANGULAR] FIR filtering completed\n");
    return 0;
}
...
void save_data(const char* filepath, const float* data, int num_points) {
    ...
    if (fp) {
        ...
        fclose(fp);
        printf("[DSP_TRIANGULAR] Filtered data saved to: %s\n", filepath);
    } else {
        perror("[DSP_TRIANGULAR] Failed to save filtered data");
    }
}
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        fprintf(stderr, "[DSP_TRIANGULAR] Usage: %s <input_file.txt>\n", argv[0]);
        fprintf(stderr, "[DSP_TRIANGULAR] Window Type: TRIANGULAR (Bartlett)\n");
        fprintf(stderr, "[DSP_TRIANGULAR] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
        return 1;
    }

    const char* input_filepath = argv[1];
    printf("[DSP_TRIANGULAR] ========================================\n");
    printf("[DSP_TRIANGULAR] Analyzing file: %s\n", input_filepath);
    printf("[DSP_TRIANGULAR] Window Type: TRIANGULAR\n");
    printf("[DSP_TRIANGULAR] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
    printf("[DSP_TRIANGULAR] ========================================\n");

    FILE* f = fopen(input_filepath, "r");
    if (!f) {
        perror("[DSP_TRIANGULAR] Failed to open input file");
        return 1;
    }
    ...
    if (!temp_adc_values) {
        perror("[DSP_TRIANGULAR] Memory allocation failed");
        fclose(f);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (current_adc_count >= temp_adc_capacity) {
            ...
            if (!new_temp) {
                perror("[DSP_TRIANGULAR] Reallocation failed");
    ...
    if (current_adc_count == 0) {
        fprintf(stderr, "[DSP_TRIANGULAR] No valid ADC data found in the file.\n");
        free(temp_adc_values);
        return 1;
    }

    printf("[DSP_TRIANGULAR] Read %d ADC samples from file\n", current_adc_count);
    ...
    if (!raw_weights || !filtered_weights || !fir_coeffs) {
        perror("[DSP_TRIANGULAR] Memory allocation for data arrays failed");
        free(temp_adc_values);
    ...
    // Generate FIR coefficients with TRIANGULAR window
    generate_fir_coeffs_triangular(fir_coeffs, NUMTAPS, CUTOFF_FREQ, SAMPLING_FREQ);

    // Normalize ADC values
    normalize_to_weights(temp_adc_values, current_adc_count, raw_weights);
    printf("[DSP_TRIANGULAR] Normalized ADC values to weights\n");
    ...
    snprintf(output_filepath, sizeof(output_filepath), "filtered_triangular_%s", 
    ...
    printf("[DSP_TRIANGULAR] ========================================\n");
    printf("[DSP_TRIANGULAR] Analysis complete!\n");
    printf("[DSP_TRIANGULAR] ========================================\n");
