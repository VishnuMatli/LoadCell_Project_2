// signal_analyzer_hamming.c: DSP analysis with HAMMING WINDOW applied to FIR coefficients
// Window Type: HAMMING - Good spectral leakage reduction with reasonable side lobes
...
// Generate FIR coefficients with HAMMING window
// Hamming window: w(n) = 0.54 - 0.46*cos(2π*n/(N-1))
void generate_fir_coeffs_hamming(float* coeffs, int num_taps, 
    ...
    printf("[DSP_HAMMING] Generating FIR coefficients with HAMMING window\n");
    printf("[DSP_HAMMING] Cutoff: %.2f Hz, Sampling: %.2f Hz, Normalized fc: %.4f\n", 
    ...
    for (int n = 0; n < num_taps; n++) {
        ...
        // HAMMING WINDOW: w(n) = 0.54 - 0.46*cos(2π*n/(N-1))
        float hamming_window = 0.54f - 0.46f * arm_cos_f32(2.0f * PI * (float)n / (num_taps - 1));
        
        // Apply window to sinc
        coeffs[n] = sinc_val * hamming_window;
        gain += coeffs[n];
    }
    ...
    printf("[DSP_HAMMING] FIR coefficients generated and normalized\n");
}

// DSP Functions
int normalize_to_weights(const int* values, int num_values, float* weights_out) {
    if (!values || !weights_out || num_values <= 0) {
        fprintf(stderr, "[DSP_HAMMING] Error: Invalid input to normalize_to_weights.\n");
        return -1;
    }
...
int fir_filter(const float* values, int num_values, float* filtered_values_out,
    ...
    if (!values || !filtered_values_out || !fir_coeffs || num_values <= 0) {
        fprintf(stderr, "[DSP_HAMMING] Error: Invalid input to fir_filter.\n");
        return -1;
    }

    if (num_values < NUMTAPS) {
        fprintf(stderr, "[DSP_HAMMING] Warning: Data length (%d) < NUMTAPS (%d). Skipping filter.\n", 
                num_values, NUMTAPS);
        return 0;
    }
    ...
    if (!state) {
        fprintf(stderr, "[DSP_HAMMING] Error: Memory allocation failed for FIR state buffer.\n");
        return -1;
    }
    ...
    // Clean up
    free(state);
    printf("[DSP_HAMMING] FIR filtering completed\n");
    return 0;
}
...
void save_data(const char* filepath, const float* data, int num_points) {
    ...
    if (fp) {
        ...
        fclose(fp);
        printf("[DSP_HAMMING] Filtered data saved to: %s\n", filepath);
    } else {
        perror("[DSP_HAMMING] Failed to save filtered data");
    }
}
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        fprintf(stderr, "[DSP_HAMMING] Usage: %s <input_file.txt>\n", argv[0]);
        fprintf(stderr, "[DSP_HAMMING] Window Type: HAMMING\n");
        fprintf(stderr, "[DSP_HAMMING] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
        return 1;
    }

    const char* input_filepath = argv[1];
    printf("[DSP_HAMMING] ========================================\n");
    printf("[DSP_HAMMING] Analyzing file: %s\n", input_filepath);
    printf("[DSP_HAMMING] Window Type: HAMMING\n");
    printf("[DSP_HAMMING] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
    printf("[DSP_HAMMING] ========================================\n");

    FILE* f = fopen(input_filepath, "r");
    if (!f) {
        perror("[DSP_HAMMING] Failed to open input file");
        return 1;
    }
    ...
    if (!temp_adc_values) {
        perror("[DSP_HAMMING] Memory allocation failed");
        fclose(f);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (current_adc_count >= temp_adc_capacity) {
            ...
            if (!new_temp) {
                perror("[DSP_HAMMING] Reallocation failed");
    ...
    if (current_adc_count == 0) {
        fprintf(stderr, "[DSP_HAMMING] No valid ADC data found in the file.\n");
        free(temp_adc_values);
        return 1;
    }

    printf("[DSP_HAMMING] Read %d ADC samples from file\n", current_adc_count);
    ...
    if (!raw_weights || !filtered_weights || !fir_coeffs) {
        perror("[DSP_HAMMING] Memory allocation for data arrays failed");
        free(temp_adc_values);
    ...
    // Generate FIR coefficients with HAMMING window
    generate_fir_coeffs_hamming(fir_coeffs, NUMTAPS, CUTOFF_FREQ, SAMPLING_FREQ);

    // Normalize ADC values
    normalize_to_weights(temp_adc_values, current_adc_count, raw_weights);
    printf("[DSP_HAMMING] Normalized ADC values to weights\n");
    ...
    snprintf(output_filepath, sizeof(output_filepath), "filtered_hamming_%s", 
    ...
    printf("[DSP_HAMMING] ========================================\n");
    printf("[DSP_HAMMING] Analysis complete!\n");
    printf("[DSP_HAMMING] ========================================\n");
