// signal_analyzer_parks_mcclellan.c: DSP analysis using Parks-McClellan style FIR design
// Method: PARKS-MCCLELLAN (IRLS approximation) - near-equiripple behavior
...
static float evaluate_amplitude(const float* a, int M, float w) {
    float amp = a[0];
    for (int k = 1; k <= M; ++k) {
        amp += 2.0f * a[k] * cosf((float)k * w);
    }
    return amp;
}

// Generate FIR coefficients using Parks-McClellan style IRLS approximation
void generate_fir_coeffs_parks(float* coeffs, int num_taps,
                               float cutoff_freq, float sampling_freq) {
    float fc = cutoff_freq / sampling_freq;
    int M = (num_taps - 1) / 2;
    int unknowns = M + 1;
    float a[8] = {0.0f};
    float weights[GRID_POINTS];

    printf("[DSP_PARKS] Generating FIR coefficients with PARKS-MCCLELLAN style design\n");
    printf("[DSP_PARKS] Cutoff: %.2f Hz, Sampling: %.2f Hz, Normalized fc: %.4f\n",
           cutoff_freq, sampling_freq, fc);

    for (int i = 0; i < GRID_POINTS; ++i) {
        float w = PI * (float)i / (float)(GRID_POINTS - 1);
        float f_norm = w / (2.0f * PI);
        weights[i] = (f_norm <= fc) ? 1.0f : 2.0f;
    }
    ...
    for (int iter = 0; iter < IRLS_ITERS; ++iter) {
        ...
        for (int i = 0; i < GRID_POINTS; ++i) {
            ...
            for (int r = 0; r < unknowns; ++r) {
                ATd[r] += weights[i] * phi[r] * desired;
                for (int c = 0; c < unknowns; ++c) {
                    ATA[r * unknowns + c] += weights[i] * phi[r] * phi[c];
                }
            }
        }

        if (solve_linear_system(ATA, ATd, next_a, unknowns) != 0) {
            fprintf(stderr, "[DSP_PARKS] Warning: IRLS solve failed at iteration %d\n", iter + 1);
            break;
        }
        ...
        for (int i = 0; i < GRID_POINTS; ++i) {
            ...
            float f_norm = w / (2.0f * PI);
            float desired = (f_norm <= fc) ? 1.0f : 0.0f;
            float err = desired - evaluate_amplitude(a, M, w);
            float band_base = (f_norm <= fc) ? 1.0f : 2.0f;
            weights[i] = band_base / fmaxf(fabsf(err), 0.02f);
            if (weights[i] > 80.0f) {
                weights[i] = 80.0f;
            }
        }
    ...
    printf("[DSP_PARKS] FIR coefficients generated and normalized\n");
}

int normalize_to_weights(const int* values, int num_values, float* weights_out) {
    if (!values || !weights_out || num_values <= 0) {
        fprintf(stderr, "[DSP_PARKS] Error: Invalid input to normalize_to_weights.\n");
        return -1;
    }
...
int fir_filter(const float* values, int num_values, float* filtered_values_out,
    ...
    if (!values || !filtered_values_out || !fir_coeffs || num_values <= 0) {
        fprintf(stderr, "[DSP_PARKS] Error: Invalid input to fir_filter.\n");
        return -1;
    }

    if (num_values < NUMTAPS) {
        fprintf(stderr, "[DSP_PARKS] Warning: Data length (%d) < NUMTAPS (%d). Skipping filter.\n",
                num_values, NUMTAPS);
        return 0;
    }
    ...
    if (!state) {
        fprintf(stderr, "[DSP_PARKS] Error: Memory allocation failed for FIR state buffer.\n");
        return -1;
    }
    ...
    free(state);
    printf("[DSP_PARKS] FIR filtering completed\n");
    return 0;
}
...
void save_data(const char* filepath, const float* data, int num_points) {
    ...
    if (fp) {
        ...
        fclose(fp);
        printf("[DSP_PARKS] Filtered data saved to: %s\n", filepath);
    } else {
        perror("[DSP_PARKS] Failed to save filtered data");
    }
}

int main(int argc, char* argv[]) {
    if (argc < 2) {
        fprintf(stderr, "[DSP_PARKS] Usage: %s <input_file.txt>\n", argv[0]);
        fprintf(stderr, "[DSP_PARKS] Method: PARKS-MCCLELLAN\n");
        fprintf(stderr, "[DSP_PARKS] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
        return 1;
    }

    const char* input_filepath = argv[1];
    printf("[DSP_PARKS] ========================================\n");
    printf("[DSP_PARKS] Analyzing file: %s\n", input_filepath);
    printf("[DSP_PARKS] Method: PARKS-MCCLELLAN\n");
    printf("[DSP_PARKS] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
    printf("[DSP_PARKS] ========================================\n");

    FILE* f = fopen(input_filepath, "r");
    if (!f) {
        perror("[DSP_PARKS] Failed to open input file");
        return 1;
    }
    ...
    if (!temp_adc_values) {
        perror("[DSP_PARKS] Memory allocation failed");
        fclose(f);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (current_adc_count >= temp_adc_capacity) {
            ...
            if (!new_temp) {
                perror("[DSP_PARKS] Reallocation failed");
    ...
    if (current_adc_count == 0) {
        fprintf(stderr, "[DSP_PARKS] No valid ADC data found in the file.\n");
        free(temp_adc_values);
        return 1;
    }

    printf("[DSP_PARKS] Read %d ADC samples from file\n", current_adc_count);
    ...
    if (!raw_weights || !filtered_weights || !fir_coeffs) {
        perror("[DSP_PARKS] Memory allocation for data arrays failed");
        free(temp_adc_values);
    ...
    generate_fir_coeffs_parks(fir_coeffs, NUMTAPS, CUTOFF_FREQ, SAMPLING_FREQ);
    normalize_to_weights(temp_adc_values, current_adc_count, raw_weights);
    printf("[DSP_PARKS] Normalized ADC values to weights\n");

    fir_filter(raw_weights, current_adc_count, filtered_weights, fir_coeffs);

    char output_filepath[PATH_MAX];
    snprintf(output_filepath, sizeof(output_filepath), "filtered_parks_mcclellan_%s",
    ...
    printf("[DSP_PARKS] ========================================\n");
    printf("[DSP_PARKS] Analysis complete!\n");
    printf("[DSP_PARKS] ========================================\n");
