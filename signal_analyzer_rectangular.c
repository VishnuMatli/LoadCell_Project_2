// signal_analyzer_rectangular.c: DSP analysis with RECTANGULAR WINDOW applied to FIR coefficients
// Window Type: RECTANGULAR (Boxcar) - Minimal spectral leakage for sinusoids, poor side lobes
...
// Generate FIR coefficients with RECTANGULAR window
// Rectangular window: w(n) = 1.0 (all ones - no windowing)
void generate_fir_coeffs_rectangular(float* coeffs, int num_taps, 
    ...
    printf("[DSP_RECTANGULAR] Generating FIR coefficients with RECTANGULAR window\n");
    printf("[DSP_RECTANGULAR] Cutoff: %.2f Hz, Sampling: %.2f Hz, Normalized fc: %.4f\n", 
    ...
    for (int n = 0; n < num_taps; n++) {
        ...
        // RECTANGULAR WINDOW: w(n) = 1.0 (all ones)
        float rectangular_window = 1.0f;
        
        // Apply window to sinc
        coeffs[n] = sinc_val * rectangular_window;
        gain += coeffs[n];
    }
    ...
    printf("[DSP_RECTANGULAR] FIR coefficients generated and normalized\n");
}

// DSP Functions
int normalize_to_weights(const int* values, int num_values, float* weights_out) {
    if (!values || !weights_out || num_values <= 0) {
        fprintf(stderr, "[DSP_RECTANGULAR] Error: Invalid input to normalize_to_weights.\n");
        return -1;
    }
...
int fir_filter(const float* values, int num_values, float* filtered_values_out,
    ...
    if (!values || !filtered_values_out || !fir_coeffs || num_values <= 0) {
        fprintf(stderr, "[DSP_RECTANGULAR] Error: Invalid input to fir_filter.\n");
        return -1;
    }

    if (num_values < NUMTAPS) {
        fprintf(stderr, "[DSP_RECTANGULAR] Warning: Data length (%d) < NUMTAPS (%d). Skipping filter.\n", 
                num_values, NUMTAPS);
        return 0;
    }
    ...
    if (!state) {
        fprintf(stderr, "[DSP_RECTANGULAR] Error: Memory allocation failed for FIR state buffer.\n");
        return -1;
    }
    ...
    // Clean up
    free(state);
    printf("[DSP_RECTANGULAR] FIR filtering completed\n");
    return 0;
}
...
void save_data(const char* filepath, const float* data, int num_points) {
    ...
    if (fp) {
        ...
        fclose(fp);
        printf("[DSP_RECTANGULAR] Filtered data saved to: %s\n", filepath);
    } else {
        perror("[DSP_RECTANGULAR] Failed to save filtered data");
    }
}
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        fprintf(stderr, "[DSP_RECTANGULAR] Usage: %s <input_file.txt>\n", argv[0]);
        fprintf(stderr, "[DSP_RECTANGULAR] Window Type: RECTANGULAR (Boxcar)\n");
        fprintf(stderr, "[DSP_RECTANGULAR] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
        return 1;
    }

    const char* input_filepath = argv[1];
    printf("[DSP_RECTANGULAR] ========================================\n");
    printf("[DSP_RECTANGULAR] Analyzing file: %s\n", input_filepath);
    printf("[DSP_RECTANGULAR] Window Type: RECTANGULAR\n");
    printf("[DSP_RECTANGULAR] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
    printf("[DSP_RECTANGULAR] ========================================\n");

    FILE* f = fopen(input_filepath, "r");
    if (!f) {
        perror("[DSP_RECTANGULAR] Failed to open input file");
        return 1;
    }
    ...
    if (!temp_adc_values) {
        perror("[DSP_RECTANGULAR] Memory allocation failed");
        fclose(f);
        return 1;
    }

    while (fgets(line, sizeof(line), f)) {
        // Accept lines like "12345", "ADC:12345", "adc=12345", or any line containing a numeric token
        char *p = line;
        while (*p && (*p == ' ' || *p == '\t' || *p == '\r' || *p == '\n')) p++;
    ...
    if (current_adc_count == 0) {
        fprintf(stderr, "[DSP_RECTANGULAR] No valid ADC data found in the file.\n");
        free(temp_adc_values);
        return 1;
    }

    printf("[DSP_RECTANGULAR] Read %d ADC samples from file\n", current_adc_count);
    ...
    if (!raw_weights || !filtered_weights || !fir_coeffs) {
        perror("[DSP_RECTANGULAR] Memory allocation for data arrays failed");
        free(temp_adc_values);
    ...
    // Generate FIR coefficients with RECTANGULAR window
    generate_fir_coeffs_rectangular(fir_coeffs, NUMTAPS, CUTOFF_FREQ, SAMPLING_FREQ);

    // Normalize ADC values
    normalize_to_weights(temp_adc_values, current_adc_count, raw_weights);
    printf("[DSP_RECTANGULAR] Normalized ADC values to weights\n");
    ...
    snprintf(output_filepath, sizeof(output_filepath), "filtered_rectangular_%s", 
    ...
    printf("[DSP_RECTANGULAR] ========================================\n");
    printf("[DSP_RECTANGULAR] Analysis complete!\n");
    printf("[DSP_RECTANGULAR] ========================================\n");
