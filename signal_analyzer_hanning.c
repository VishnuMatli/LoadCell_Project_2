// signal_analyzer_hanning.c: DSP analysis with HANNING WINDOW applied to FIR coefficients
// Window Type: HANNING (Hann) - Better side lobe rolloff than Hamming
...
// Generate FIR coefficients with HANNING window
// Hanning window: w(n) = 0.5 - 0.5*cos(2π*n/(N-1))
void generate_fir_coeffs_hanning(float* coeffs, int num_taps, 
    ...
    printf("[DSP_HANNING] Generating FIR coefficients with HANNING window\n");
    printf("[DSP_HANNING] Cutoff: %.2f Hz, Sampling: %.2f Hz, Normalized fc: %.4f\n", 
    ...
    for (int n = 0; n < num_taps; n++) {
        ...
        // HANNING WINDOW: w(n) = 0.5 - 0.5*cos(2π*n/(N-1))
        float hanning_window = 0.5f - 0.5f * arm_cos_f32(2.0f * PI * (float)n / (num_taps - 1));
        
        // Apply window to sinc
        coeffs[n] = sinc_val * hanning_window;
        gain += coeffs[n];
    }
    ...
    printf("[DSP_HANNING] FIR coefficients generated and normalized\n");
}

// DSP Functions
int normalize_to_weights(const int* values, int num_values, float* weights_out) {
    if (!values || !weights_out || num_values <= 0) {
        fprintf(stderr, "[DSP_HANNING] Error: Invalid input to normalize_to_weights.\n");
        return -1;
    }
...
int fir_filter(const float* values, int num_values, float* filtered_values_out,
    ...
    if (!values || !filtered_values_out || !fir_coeffs || num_values <= 0) {
        fprintf(stderr, "[DSP_HANNING] Error: Invalid input to fir_filter.\n");
        return -1;
    }

    if (num_values < NUMTAPS) {
        fprintf(stderr, "[DSP_HANNING] Warning: Data length (%d) < NUMTAPS (%d). Skipping filter.\n", 
                num_values, NUMTAPS);
        return 0;
    }
    ...
    if (!state) {
        fprintf(stderr, "[DSP_HANNING] Error: Memory allocation failed for FIR state buffer.\n");
        return -1;
    }
    ...
    // Clean up
    free(state);
    printf("[DSP_HANNING] FIR filtering completed\n");
    return 0;
}
...
void save_data(const char* filepath, const float* data, int num_points) {
    ...
    if (fp) {
        ...
        fclose(fp);
        printf("[DSP_HANNING] Filtered data saved to: %s\n", filepath);
    } else {
        perror("[DSP_HANNING] Failed to save filtered data");
    }
}
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        fprintf(stderr, "[DSP_HANNING] Usage: %s <input_file.txt>\n", argv[0]);
        fprintf(stderr, "[DSP_HANNING] Window Type: HANNING\n");
        fprintf(stderr, "[DSP_HANNING] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
        return 1;
    }

    const char* input_filepath = argv[1];
    printf("[DSP_HANNING] ========================================\n");
    printf("[DSP_HANNING] Analyzing file: %s\n", input_filepath);
    printf("[DSP_HANNING] Window Type: HANNING\n");
    printf("[DSP_HANNING] Cutoff Frequency: %.2f Hz\n", CUTOFF_FREQ);
    printf("[DSP_HANNING] ========================================\n");

    FILE* f = fopen(input_filepath, "r");
    if (!f) {
        perror("[DSP_HANNING] Failed to open input file");
        return 1;
    }
    ...
    if (!temp_adc_values) {
        perror("[DSP_HANNING] Memory allocation failed");
        fclose(f);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (current_adc_count >= temp_adc_capacity) {
            ...
            if (!new_temp) {
                perror("[DSP_HANNING] Reallocation failed");
    ...
    if (current_adc_count == 0) {
        fprintf(stderr, "[DSP_HANNING] No valid ADC data found in the file.\n");
        free(temp_adc_values);
        return 1;
    }

    printf("[DSP_HANNING] Read %d ADC samples from file\n", current_adc_count);
    ...
    if (!raw_weights || !filtered_weights || !fir_coeffs) {
        perror("[DSP_HANNING] Memory allocation for data arrays failed");
        free(temp_adc_values);
    ...
    // Generate FIR coefficients with HANNING window
    generate_fir_coeffs_hanning(fir_coeffs, NUMTAPS, CUTOFF_FREQ, SAMPLING_FREQ);

    // Normalize ADC values
    normalize_to_weights(temp_adc_values, current_adc_count, raw_weights);
    printf("[DSP_HANNING] Normalized ADC values to weights\n");
    ...
    snprintf(output_filepath, sizeof(output_filepath), "filtered_hanning_%s", 
    ...
    printf("[DSP_HANNING] ========================================\n");
    printf("[DSP_HANNING] Analysis complete!\n");
    printf("[DSP_HANNING] ========================================\n");
